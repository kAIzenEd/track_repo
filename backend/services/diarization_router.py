...
def diarize_with_pyannote(audio_path: str) -> List[Dict]:
    ...
    print(f"Calling diarize service: {diarize_url} (audio_path={audio_path})")
    ...
    try:
        ...
        print(f"Diarize service responded: HTTP {response.status_code}")

        if response.status_code != 200:
            raise DiarizationError(
                f"Diarize service error (HTTP {response.status_code}): {response.text}"
            )
